...
def ensure_absent(path):
    ...
    if status == 200:
        ...
        full = os.path.join(os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store"),
...
def test_delete_symlink_to_file():
    ...
    try:
        os.symlink(
            os.path.join(os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store"), "real_target.txt"),
            os.path.join(os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store"), "symlink_to_file.txt")
        )

        # Now attempt DELETE on the symlink: should get 403 Forbidden
        request("DELETE", test_path, expected=403)
        
        real_path = os.path.join(os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store"), "real_target.txt")
...
def test_delete_deep_nested_file():
    ...
    # Setup directories
    base = os.path.join("test_webserv/tester/data/upload_store/deep/nested/dir")
    ...
    # Cleanup
    try:
        shutil.rmtree("test_webserv/tester/data/upload_store/deep")
...
def test_delete_directory_traversal_arbitrary():
    ...
    upload_dir = os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store")
...
def test_delete_static_file():
    ...
    local_dir = os.path.join("test_webserv/tester/data/dir")
...
def test_delete_percent_encoded_nested_path():
    ...
    full_dir = os.path.join("test_webserv/tester/data/upload_store/deep/nested/dir")
...
def test_delete_symlink_to_directory():
    ...
    (e.g., test_webserv/tester/data/dir), then DELETE on “/upload_store/symlink_dir” should be 403 
    and the directory behind it must remain untouched.
    """
    # Ensure file.txt exists
    file_txt_path = "test_webserv/tester/data/dir/file.txt"
    ...
    link_path_local = os.path.join(os.getenv("UPLOAD_DIR", "./test_webserv/tester/data/upload_store"), "symlink_dir")
    if os.path.islink(link_path_local):
        os.unlink(link_path_local)

    # Create the symlink
    try:
        os.symlink(os.path.abspath("test_webserv/tester/data/dir"), link_path_local)
    except OSError:
