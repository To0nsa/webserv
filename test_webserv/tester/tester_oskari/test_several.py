...
def test_get_root():
    ...
    response = requests.get("http://127.0.0.2:8080/index.html")
...
def test_oldDir_redirect():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/oldDir/", allow_redirects=False)
...
def test_newDir_directory_listing():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/newDir/")
    assert response.status_code == 200
...
def test_images_get():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/images/")
    assert response.status_code == 200

def test_images_post():    
    files = {'file': ('filename.txt', b"dummy data\n")}
    response = requests.post("http://127.0.0.2:8080/images/", files=files)
...
def test_images_delete():
    ...
    base_dir = Path("test_webserv/tester/tester_oskari/www/images")
    test_file = base_dir / "random.file"
    base_dir.mkdir(parents=True, exist_ok=True)
    test_file.write_text("Temporary file content.")

    # Run DELETE request
    response = requests.delete("http://127.0.0.2:8080/images/random.file")
...
def test_imagesREDIR():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/imagesREDIR/", allow_redirects=False)
...
def test_cgi_empty_redirect():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/cgi/empty/", allow_redirects=False)
    assert response.status_code == 307
    location = response.headers.get("Location", "")
...
def test_bad_http_request():
    ...
    # Connect to the local server at 127.0.0.2:8080
    with socket.create_connection(("127.0.0.2", 8080), timeout=5) as sock:
...
def test_not_found_error():
    ...
    """
    response = requests.get("http://127.0.0.2:8080/nonexistent.html")
...
def test_client_body_size_exceeded():
    ...
    payload = b"x" * 6000000
    try:
        response = requests.post("http://127.0.0.2:8080/images/", data=payload)
...
def test_file_upload_and_check():
    ...
    upload_path = Path(__file__).parent.parent.parent / "tester/tester_oskari/www/images" / filename
...
async def test_repeated_requests():
    ...
    """
    num_requests = 1000
    url = "http://127.0.0.2:8080/index.html"
...
async def test_concurrent_get_and_post():
    ...
    get_url = "http://127.0.0.2:8080/index.html"
    post_url = "http://127.0.0.2:8080/images/"
    ...
    uploaded_file = Path("/home/irychkov/Desktop/webserv_team/test_webserv/tester/tester_oskari/www/images/filename.txt")
    uploaded_file.unlink(missing_ok=True)

@pytest.mark.asyncio
async def test_single_post_upload():
    post_url = "http://127.0.0.2:8080/uploads/"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        uploaded_file = Path("/home/irychkov/Desktop/webserv_team/test_webserv/tester/tester_oskari/www/testfile.txt")
        uploaded_file.unlink(missing_ok=True)
...
async def test_repeated_post_requests_from_multiple_clients():
    post_url = "http://127.0.0.2:8080/cgi/test.py"
    data = "Test content\n"
...
def test_keep_alive_connection():
    ...
    with socket.create_connection(("127.0.0.2", 8080)) as sock:
...
def test_path_traversal_attack():
    response = requests.get("http://127.0.0.2:8080/../../etc/passwd")
...
def test_extra_headers():
    ...
    response = requests.get("http://127.0.0.2:8080/index.html", headers=headers)
    assert response.status_code == 200

def test_unsupported_method():
    response = requests.request("PATCH", "http://127.0.0.2:8080/index.html")
...
def test_dos_simulation():
    ...
    try:
        for _ in range(100):
            sock = socket.create_connection(("127.0.0.2", 8080), timeout=1)
            sockets.append(sock)
...
def test_cgi_script():
    url = "http://127.0.0.2:8080/cgi/test.py"
...
def test_cgi_get_query():
    url = "http://127.0.0.2:8080/cgi/test.py?foo=bar"
    response = requests.get(url)
...
def send_chunked_request():
    ...
    client_socket.connect(('127.0.0.2', 8080))
