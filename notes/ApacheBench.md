...
| ------------------ | ------------------------------------------------------------ |
| 🔁 Longer duration | `ab -n 10000 -c 200 http://localhost:8001/index.html`         |
| 📂 Big files       | `ab -n 1000 -c 50 http://localhost:8001/huge.mp4`            |  
|                    | _Generate file:_ `dd if=/dev/urandom of=huge.mp4 bs=1M count=100` |
| 🔧 CGI load        | `ab -n 1000 -c 50 http://localhost:8001/cgi-bin/test.py`      |
| 🧪 POST test       | `ab -n 500 -c 50 -p post.txt -T text/plain http://localhost:8001/post_body` |
| 🧵 Keep-alive test | `ab -k -n 1000 -c 100 http://localhost:8001/`                |
...
| Target            | Tool / Idea                                              |
| ----------------- | -------------------------------------------------------- |
| 🔁 **Keep-alive** | `ab -k -n 5000 -c 100 http://localhost:8001/`            |
...
| 🔁 **Repeated**   | `wrk -t4 -c200 -d30s http://localhost:8001/`             |

---

## Notes

- Make sure your server is running and listening on port 8001.
