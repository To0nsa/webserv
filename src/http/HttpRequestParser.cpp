...
bool parseReqBody(HttpRequest& req, const std::string& bodyPart, std::size_t clientMaxBodySize,
    ...
    if (transferEncoding == "chunked") {
        if (!isChunkedBodyComplete(bodyPart)) {
            // Logger::logFrom(LogLevel::INFO, "HttpRequestParser", "Incomplete chunked body, server
            // reads again");
            errorCode = 0;
            return false;
        }
    ...
    if (bodyPart.size() < contentLength) {
        // Logger::logFrom(LogLevel::INFO, "HttpRequestParser", "Incomplete request body, server
        // reads again");
        errorCode = 0;
        return false;
    }
